...
rja_baseline<-with(df_et_agg[df_et_agg$t %in% c('T2'),],
                   fun_return_descriptive(rja_occurances/16,rndm))
...
n_timepoint<-sapply(with(df_sample,by(t,rndm,table)),
                    function(x){paste(x[1],'/',x[2],'/',x[3],'/',x[4])})
...
rja_T2<-with(df_et_agg[df_et_agg$t %in% c('T2') & df_et_agg$rndm!='non autistic',],
                   fun_return_descriptive(rja_occurances/16,rndm))[1:2]
...
table_sample<-descriptives_table %>%
  kbl(caption = "Table 2. Sample description: autistic groups across timepoints.",
      col.names = c('','timepoints','A-FFIP - autistic individuals','TAU - autistic individuals'),
      row.names = F) %>%
  kable_classic(full_width = F, html_font = "Cambria") %>%
  #adds solid line after every row
  row_spec(c(2,seq(2, nrow(descriptives_table), 4)[-c(1)]), extra_css = "border-bottom: 1px solid;")
...
ggplot(df_escs,aes(x=rja_any,y=Total_RJA))+geom_point()+geom_smooth(method='lm')
...
tiff(file=paste0(project_path,"/output/figures/figure_stimulus_material.tiff"), 
     width=9, height=12, units="in", res=300, compression='lzw') 
...
## supplement - model overview

```{r}

#RJA likelihood - intervention model
glmm<-glmer(rja_any~t*rndm+
            (1|pic)+(1|index_trial),data=df_trial[df_trial$t %in% c('T2','T6') & 
                                                    df_trial$rndm!='non autistic',],family='binomial')
...
table_data_model<-round(car::Anova(glmm,type="III"),3)

##supplements - full table
  row_names<-c('intercept','timepoint (T)','group (G)',
               'T x G')
  ...
  table_data_model %>%
    kbl(caption = "Generalized linear mixed model of RJA likelihood",
