...
list_jointatt<-lapply(df,function(x){x[grepl('jointattention_',x$eventlog),]})

#remove useless data
list_jointatt<-lapply(list_jointatt,function(x){x[!grepl('jointattention_blockA',x$eventlog),]})
list_jointatt<-lapply(list_jointatt,function(x){x[!grepl('jointattention_blockB',x$eventlog),]})
...
fun_define_trials<-function(block_data){
  ...
  event.change<-lapply(block_data,function(x){which(diff(as.numeric(as.factor(x$eventlog)))!=0)}) #index event change per participant
  ...
  block_data<-lapply(block_data,fun_rename,variable_position=30,new_name='index_trial')
  block_data<-lapply(block_data,fun_rename,variable_position=31,new_name='ts_event')
  return(block_data)
}
...
list_jointatt<-lapply(list_jointatt,function(x){x[x$index_trial<=16,]})
...
##--> after matching: K = 44, T2 = 51, T4 = 44, T6 = 32

#missing data
table(is.na(df_jointatt$rpd))[2]/sum(table(is.na(df_jointatt$rpd))) #--> 46.2 missing data in pupil data
table(is.na(df_jointatt$gazepos_x))[2]/sum(table(is.na(df_jointatt$gazepos_x))) #--> 54.4% missing data in gaze x
table(is.na(df_jointatt$gazepos_y))[2]/sum(table(is.na(df_jointatt$gazepos_y))) #--> 59.2% missing data in gaze y
...
ggplot(df_jointatt[sample(1:nrow(df_jointatt),size=nrow(df_jointatt)/100),],aes(x=ts_event,y=rpd))+
...
#--> between condition
ggplot(df_jointatt[sample(1:nrow(df_jointatt),size=nrow(df_jointatt)/10),],aes(x=ts_event,y=rpd,group=condition,color=condition))+
  geom_smooth()+theme_bw()+labs(title='pupil size within trial',x='time (samples)',y='pupil size (mm)')+
  geom_vline(xintercept=450)+geom_vline(xintercept=900)
...
save(df_jointatt, df_trial, all.match, file=paste0(home_path,project_path,"/data/all_data_preprocessed_FINAL_jointatt_101022.Rdata"))
  ...
  # groupdiff_p<-c(NA,
  #                round(as.numeric(with(df_timepoint,chisq.test(timepoint,Geschlecht_Index))['p.value']),3),
  #                round(summary(with(df_timepoint,aov(test_age~timepoint)))[[1]][1,'Pr(>F)'],3),
  #                round(summary(with(df_timepoint,aov(age_months~timepoint)))[[1]][1,'Pr(>F)'],3),
  #                round(summary(with(df_timepoint,aov(IQ~timepoint)))[[1]][1,'Pr(>F)'],3),
  #                round(summary(with(df_timepoint,aov(srs_sum~timepoint)))[[1]][1,'Pr(>F)'],3),
  #                round(summary(with(df_timepoint,aov(RBSR_ges~timepoint)))[[1]][1,'Pr(>F)'],3),
  #                round(summary(with(df_timepoint,aov(CBCL_T_GES~timepoint)))[[1]][1,'Pr(>F)'],3),
  #                round(summary(with(df_trial,aov(rja_duration~timepoint)))[[1]][1,'Pr(>F)'],3),
  #                round(summary(with(df_trial,aov(fixation_duration~timepoint)))[[1]][1,'Pr(>F)'],3),
  #                round(summary(with(df_trial,aov(rpd_middle~timepoint)))[[1]][1,'Pr(>F)'],3),
  #                round(summary(with(df_trial,aov(no_data~timepoint)))[[1]][1,'Pr(>F)'],3)
  # )
  ...
  table_sample<-descriptives_table %>%
    kbl(caption = "Sample description",
        col.names = c('','NTC','ASD (T2)','ASD (T4)','ASD (T6)'),
        row.names = F) %>%
 ...
 #LINEAR MIXED MODELS ####
 with(df_trial,table(rja_any,timepoint))
 ...
 ##--> RJA - main findings - on per-trial level ####
  ...
  ##--> pupillary response by timepoint and condition ####
    ...
    ggplot(df_trial,aes(x=condition,y=rpd_middle,group=timepoint,fill=timepoint))+geom_boxplot()
  ...
  #RJA
  lmm<-glmer(rja_any~scale(srs_sum)+(1|pic),data=df_trial,family=binomial(link = "logit"))
  summary(lmm)

  lmm<-glmer(rja_any~scale(RBSR_ges)+(1|pic),data=df_trial,family=binomial(link = "logit"))
  summary(lmm)

  lmm<-glmer(rja_any~scale(CBCL_T_GES)+(1|pic),data=df_trial,family=binomial(link = "logit"))
  summary(lmm)
  ...
  lmm<-glmer(rja_any~scale(baseline_pd)+condition+
  ...
  lmm<-glmer(rja_any~scale(rpd_middle)+condition+
               (1|pic),data=df_trial,family=binomial(link = "logit"))
  summary(lmm)
  ...
  lmm<-glmer(rja_any~scale(rpd_late)+timepoint+(1|pic),data=df_trial,family=binomial(link = "logit"))
  summary(lmm)
...
###-------- PRELIMINARY FINDINGS ####
  ...
  ## TODO: A COMBINED PLOT for INSAR abstract
